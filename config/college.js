module.exports = {
  name:       'Greenvaley Junior College',
  shortName:  'Greenvaley',
  tagline:    'MPC & BiPC | Class 11 & 12',
  board:      'TSBIE Affiliated',
  programs:   ['MPC', 'BiPC'],
  address:    'H. No: 3-140/2K/3, K/4, K5 Ananthasagar (V), Hasanparthy (M), Hanumakonda (D) – 506371',
  phone:      '7207904044',
  phone2:     '7207904045',
  email:      'office@gvjc.in',
  whatsapp:   '917207904045',
  website:    'https://greenvaleycollege.com',
  estYear:    2024,

  // Leadership
  directors: [
    {
      name:        'Dr. G Bharadwaja Naidu',
      role:        'Chairman',
      designation: 'Chairman, Greenvaley Junior College',
      email:       'gbn@gvjc.in',
      photo:       '/images/directors/bharadwaja.jpg',
      bio:         'Sri Dr. G. Bharadwaja Naidu is the visionary founder of the Greenwood Group of Schools and Colleges. Over three decades, he built a multi-campus network spanning CBSE schools and junior colleges across Telangana and Andhra Pradesh, with 600+ staff and thousands of alumni placed in IITs, NITs, and premier institutes. The recipient of an honorary doctorate for his service to Indian education, he established Greenvaley Junior College in 2025 as the latest step in his mission to make quality intermediate education accessible at scale.',
    },
    {
      name:        'G Aakarsh Naidu',
      role:        'Director',
      designation: 'Director, Greenvaley Junior College',
      email:       'aakarsh@gvjc.in',
      photo:       '/images/directors/aakarsh.jpg',
      bio:         'G Aakarsh Naidu holds a BSc Economics (Honours) from the University of Sussex, United Kingdom, and is an alumnus of IIM Calcutta. Returning to India with a global perspective on education and management, he leads Greenvaley Junior College as Director — overseeing academics, infrastructure, and student welfare.',
    },
  ],

  // Sister institutions under Akarshahlad Educational Society
  sisterBranches: [
    {
      name:     'Greenwood High School',
      type:     'School · Nursery – Class X',
      location: 'Hanamkonda, Telangana',
      url:      '#',
    },
    {
      name:     'Abhyaas The Global School',
      type:     'CBSE School · Nursery – Class X',
      location: 'Bhimavaram, Andhra Pradesh',
      url:      '#',
    },
    {
      name:     'Pinegrove Junior College',
      type:     'Intermediate College',
      location: 'Hyderabad, Telangana',
      url:      '#',
    },
  ],
};
